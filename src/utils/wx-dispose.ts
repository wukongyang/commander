  ...
  return new Promise((resolve,reject)=>{
        try {
            ...
            if(configOptions.length>0){
                ...
                fs.rename(process.cwd() + `${basePath}/app.wxss`, process.cwd() + `${basePath}/app.${configOptions[0]}`, () => {
                })
                fs.rename(process.cwd() + `${basePath}/components/ViscidGlobe/ViscidGlobe.wxss`, process.cwd() + `${basePath}/components/ViscidGlobe/ViscidGlobe.${configOptions[0]}`, () => {
                })
                fs.rename(process.cwd() + `${basePath}/pages/index/index.wxss`, process.cwd() + `${basePath}/pages/index/index.${configOptions[0]}`, () => {
                })
            }
            // 写入配置
